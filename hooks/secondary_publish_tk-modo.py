...
class PublishHook(Hook):
    ...
    def __publish_alembic_cache(self, item, output, work_template, primary_publish_path, 
        ...
        if start_frame and end_frame:
            alembic_args['start_frame'] = start_frame
            alembic_args['end_frame'] = end_frame

        alembic_args['publish_path'] = publish_path.replace("\\", "/")

        progress_cb(30, "Exporting Alembic cache")
        try:
            modoshotgunsupport.export_alembic(**alembic_args)
        except Exception, e:
    ...
    def _find_scene_animation_range(self):
        ...
        import modoshotgunsupport
        return modoshotgunsupport.get_scene_range()
